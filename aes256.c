...
/* -------------------------------------------------------------------------- */
void aes256_encrypt_cbc(uint8_t *buf, uint8_t len_buf, uint8_t *key, uint8_t *out)
{   // Initialization
    aes256_context ctx;
    aes256_init(&ctx, key);
    
    // IV value
    uint8_t iv[16] = {0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35};

    // Generating the temp buffer to process each 16-bytes block with num_block blocks
    uint8_t tem_buf[16];
    uint8_t num_block = ((len_buf-1) >> 4) + 1;
    uint8_t block_ind = 0;

    while (num_block > 0)
    {
	// Initializing temp buffer value to all zero	
	memset(tem_buf, 0, 16*sizeof(uint8_t));

	// Copy input buffer from specific location depends on number of the block
	if (num_block == 1)
	  memcpy(tem_buf, buf+(16*block_ind), (((len_buf - 1)%16) + 1)*sizeof(uint8_t));
	else memcpy(tem_buf, buf+(16*block_ind), 16*sizeof(uint8_t));
	...
	// Encryption
	aes256_encrypt_ecb(&ctx, tem_buf);

	// Copy the result to output. Current result will be the next iteration's IV.
	memcpy(out+(16*block_ind), tem_buf, 16*sizeof(uint8_t));	
	memcpy(iv, tem_buf, 16*sizeof(uint8_t));	
...
/* -------------------------------------------------------------------------- */
void aes256_decrypt_cbc(uint8_t *buf, uint8_t len_buf, uint8_t *key, uint8_t *out)
{   // Initialization
    aes256_context ctx;
    aes256_init(&ctx, key);
	...
	// Copy input buffer from specific location depends on number of the block
	memcpy(tem_buf, buf+(16*block_ind), 16*sizeof(uint8_t));

	// Decryption
	aes256_decrypt_ecb(&ctx, tem_buf);

	// Calculating the message by XORing with IV
	tem_buf[0] ^= iv[0];
	tem_buf[1] ^= iv[1];
	tem_buf[2] ^= iv[2];
	tem_buf[3] ^= iv[3];
	...
	// Copy the result to output. Current input will be the next iteration's IV.
	memcpy(out+(16*block_ind), tem_buf, 16*sizeof(uint8_t));	
	memcpy(iv, buf+(16*block_ind), 16*sizeof(uint8_t));	
